...
import { BlockBuilder, PageBuilder } from "../testing/mock-logseq-sdk";
			...
			setup: (mockAPI: MockLogseqAPI) => {
				...
				const blocks = [
					...
					new BlockBuilder()
						...
						.build(),
					new BlockBuilder().withContent("React is a powerful UI library").build(),
					new BlockBuilder().withContent("JavaScript is the language of the web").build(),
				];
			...
			setup: (mockAPI: MockLogseqAPI) => {
				...
				const blocks = [
					new BlockBuilder()
						...
						.withProperty("logseq.property/heading", 1)
						.build(),
					new BlockBuilder().withContent("React Hooks changed how we write components.").build(),
					new BlockBuilder()
						.withContent("## Common Hooks")
						.withProperty("logseq.property/heading", 2)
						.build(),
					new BlockBuilder().withContent("**useState** - Manages component state").build(),
					new BlockBuilder().withContent("**useEffect** - Handles side effects").build(),
					new BlockBuilder().withContent("**useCallback** - Memoizes functions").build(),
					new BlockBuilder()
	...
	parameters: {
		mockLogseq: {
			setup: (mockAPI: MockLogseqAPI) => {
				...
				const parentBlock = new BlockBuilder().withContent("Main project tasks").build();

				const childBlocks = [
					new BlockBuilder().withContent("Set up development environment").build(),
					new BlockBuilder().withContent("Implement core features").build(),
					new BlockBuilder().withContent("Write tests").build(),
				];
...
export const WithBlockReferences: Story = {
	parameters: {
		mockLogseq: {
			setup: (mockAPI: MockLogseqAPI) => {
				const page = new PageBuilder().withName("Meeting Notes").build();
				...
				const blocks = [
					new BlockBuilder()
						...
						.withProperty("logseq.property/heading", 1)
						.build(),
					new BlockBuilder().withContent("Discussed project progress and next steps.").build(),
					referencedBlock,
					new BlockBuilder().withContent("Follow up on ((ref-block-123))").build(),
				];
			...
			setup: (mockAPI: MockLogseqAPI) => {
				...
				const blocks = [
					...
					new BlockBuilder()
						...
						.build(),
					new BlockBuilder().withContent("New frameworks and tools emerge every year.").build(),
				];
...
export const EmptyPage: Story = {
	parameters: {
		mockLogseq: {
			setup: (mockAPI: MockLogseqAPI) => {
				const page = new PageBuilder().withName("Empty Page").build();
			...
			setup: (mockAPI: MockLogseqAPI) => {
				...
				const blocks = [
					...
					new BlockBuilder()
						...
						.build(),
					new BlockBuilder().withContent("**number** - Numeric values").build(),
					new BlockBuilder().withContent("**string** - Text values").build(),
					new BlockBuilder().withContent("**boolean** - True/false values").build(),
					new BlockBuilder().withContent("**array** - Collections of values").build(),
					new BlockBuilder().withContent("**tuple** - Fixed-length arrays").build(),
					new BlockBuilder().withContent("**enum** - Named constants").build(),
					new BlockBuilder().withContent("**any** - Opt-out of type checking").build(),
					new BlockBuilder().withContent("**void** - No value").build(),
					new BlockBuilder()
						.withContent("## Interfaces")
						.withProperty("logseq.property/heading", 2)
						.build(),
					new BlockBuilder().withContent("Interfaces define the structure of objects.").build(),
					new BlockBuilder()
			...
			setup: (mockAPI: MockLogseqAPI) => {
				...
				const blocks = [
					...
					new BlockBuilder()
						...
						.build(),
					new BlockBuilder().withContent("Emojis work too: 💡 🔥 ⚡ 🎯").build(),
				];
